...
// 1. What is "this" when joe.scope() is invoked? "this" refers to the instance of the Student object (joe). 
// It represents the object on which the scope method is invoked.
//
// 2. What is "this" when joe.scopeArrow() is invoked?"this" refers to the global object (typically window in a 
// browser environment). Arrow functions do not have their own "this" binding, so they inherit the "this" value from the enclosing scope, which is the global scope in this case.
//
// 3. Explain why "this" is different when an arrow function is used.In traditional functions, the value of "this" is dynamically scoped based on how the function is called. 
// However, arrow functions do not have their own "this" binding; instead, they inherit the "this" value from the enclosing scope at the time the arrow function is created. 
// This behavior makes arrow functions unsuitable for methods that rely on the dynamic value of "this," such as constructor functions or methods attached to objects. In the provided example, the arrow function joe.scopeArrow uses the global object as "this" because it is not within a traditional function with its own "this" binding.
//
